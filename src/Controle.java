...
public class Controle // classe com métodos estáticos para realizar as operacoes de Imagem
{
    ...
    public static void fftDireta(JanelaInterna jan) {
        ...
        boolean flag = fft2d(real, imag, nx, ny, 1);
        ...
        novaJanelaInterna.setLarguraImagem(nx);
        novaJanelaInterna.setAlturaImagem(ny);
        novaJanelaInterna.setMatrizReal(real);
        novaJanelaInterna.setMatrizImag(imag);
        novaJanelaInterna.setDFT(true);
        novaJanelaInterna.setTitle(jan.getTitle());
        novaJanelaInterna.mostraMagDFT();

        janela.adicionarJanelaInterna(novaJanelaInterna);
    }

    public static void fftInversa(JanelaInterna jan) {
        int i, j;
        int nx = jan.getLarguraImagem();
        int ny = jan.getAlturaImagem();
        
        if(jan.isDFT() == false) {
            JOptionPane.showMessageDialog(null, "Erro: A imagem selecionada não é uma DFT");
            return;
        }
        
        float[][] real = jan.getReal();
        float[][] imag = jan.getImag();

        fft2d(real, imag, nx, ny, -1);
        
        //descentraliza a transformada
        for (i = 0; i < nx; i++)
            for (j = 0; j < ny; j++)
                if ((i + j) % 2 == 1)
                    real[i][j] = -real[i][j];
        
        arrumaEscala(real, ny, ny);
        ...
        novaJanelaInterna.setTitle("IDFT de " + jan.getTitle());
        novaJanelaInterna.criarImagem();
    ...
    private static boolean fft2d(float realPart[][], float imagPart[][], int nx, int ny, int dir) {
        int i, j, m1, m2;
        float[] real, imag;
        ...
        for (int canal = 0; canal < 3; canal++) {
            for (int i = 0; i < jan.getLarguraImagem(); i++) {
                for (int j = 0; j < jan.getAlturaImagem(); j++) {
                    ...
                    if (matriz[canal][i][j] > 255) {
                        matriz[canal][i][j] = 255;
                    }
